...
bool binarySearchRecursive(const int *begin, const int *end, int valueToFind)
{
	...
	if ( begin < end )
	{
		...
		if( valueToFind < *mid )
			return binarySearch(begin, mid, valueToFind);
		
		return binarySearch(mid + 1, end, valueToFind);
	}
